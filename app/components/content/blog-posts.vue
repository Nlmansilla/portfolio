    ...
    <section class="not-prose font-mono">
      ...
      <ul>
        <li v-for="post in posts" :key="post._path">
          <NuxtLink :to="post._path" class="group column hover:bg-gray-100 dark:hover:bg-gray-800">
            <div
...
// Preventing double fetching if we write code as comment below
...
const posts = computed(() => {
  ...
  for (const post of data.value) {
    ...
    const shouldDisplayYear = year != lastYear
    post.displayYear = shouldDisplayYear
    post.year = year
    result.push(post)
    lastYear = year
  }
